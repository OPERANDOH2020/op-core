...
public class PolicyComputerApi  {
   private final PolicyComputerApiService delegate = PolicyComputerApiServiceFactory.getPolicyComputerApi();
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "Creates the UPP when the user registers with OPERANDO.", notes = "This is a computational resource accessed via a REST POST call to carry out a computation\non the provided inputs. This results in an resource that is created and stored in the Policy DB and can be accessed by the URL returned as a result of this operation. In OPERANDO, this method is called by the management console when a user registers with OPERANDO. The user simply enters privacy details and answers a general questionnaire. \n", response = ComputationResult.class, tags={ "Policy Compute", "POST" })
    @io.swagger.annotations.ApiResponses(value = {
        @io.swagger.annotations.ApiResponse(code = 200, message = "UPP was created. There is no invalid response - if the method is called on the operation with an existing user_id then the server updates the record field (This is not the data storage operation).\n", response = ComputationResult.class),
    ...
    public Response policyComputerPost(
        ...
        @ApiParam(value = "The set of privacy preferences. This is a JSON object with the answers to the initial questionnaire. These can be changed and updated. \n" ,required=true) List<UserPreference> generalPreferences,
