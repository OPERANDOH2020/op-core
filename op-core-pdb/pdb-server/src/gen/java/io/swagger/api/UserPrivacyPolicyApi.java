...
,@Context SecurityContext securityContext, @Context HttpHeaders headers)
    ...
    @io.swagger.annotations.ApiOperation(value = "Create a new UPP entry in the database for the user.", notes = "Called when a new user is registered with operando. Their new privacy preferences are passed in the UPP document and stored in the policy DB. ", response = String.class, tags={ "UPP","POST", })
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 201, message = "The document (UPP) was successfully created in the database. The Operando user-id is returned as a string parameter", response = String.class),
        
        @io.swagger.annotations.ApiResponse(code = 405, message = "Error. The document (UPP) at this id has previously been created in the database.", response = String.class) })
